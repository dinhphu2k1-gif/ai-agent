...
interface CreateChannelRequest {
  title: string
  icon?: string   // Material Symbols, mặc định "forum"
}
```

**Response 201** — `ApiResponse<Channel>`
...
**Lỗi:** `400 VALIDATION_ERROR` (title rỗng / quá dài).

**FE:** Sau khi tạo → `navigate(/chat/{id})` và refresh danh sách sidebar từ `GET /channels`.
...
**Response:** `204 No Content` hoặc `200` với `ApiResponse<null>`.

**Lỗi:** `404 CHANNEL_NOT_FOUND`, `403 CHANNEL_FORBIDDEN` (channel seed / không thuộc user).

**FE:** Nếu đang mở channel bị xóa → chuyển sang channel còn lại đầu tiên hoặc `/chat`.
...
| UI | Endpoint / event |
|----|------------------|
| Sidebar / header channel | `GET /channels`, `POST /channels`, `DELETE /channels/{id}` |
