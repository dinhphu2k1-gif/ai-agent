...
const ActionEffectStep = ({
  ...
  const extraActions = selectedActions
    .filter((name) => !AVAILABLE_ACTIONS.some((a) => a.name === name))
    .map((name) => ({
      name,
      icon: 'bolt',
      description: 'Current permission action',
    }))

  const actionOptions = [...AVAILABLE_ACTIONS, ...extraActions]
